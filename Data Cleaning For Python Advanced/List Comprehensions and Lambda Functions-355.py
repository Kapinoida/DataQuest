...
hn_clean = []
for i in hn:
    clear = del_key(i, 'createdAtI')
    hn_clean.append(clear)
...
def key(s_dict):
    return s_dict['numComments']

most_comments = max(hn_clean, key=key)
...
cleaned_tags = tags.apply(lambda i: i[-1] if len(i) == 4 else None)
