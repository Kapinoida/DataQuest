...
WHERE ShareWomen > .5 AND Median > 50000
...
WHERE (Major_category = 'Engineering') AND (ShareWomen > .5 OR Unemployment_rate < 0.051)

## 7. Ordering Results Using ORDER BY ##

SELECT Major, ShareWomen, Unemployment_rate FROM recent_grads
WHERE ShareWomen > .3 AND Unemployment_rate < .1
