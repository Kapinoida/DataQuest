...
for c in countries:
    ...
    sorted_rows = selected_rows.sort_values("employees", ascending=False)
    first_employer = sorted_rows.iloc[0]
    employer_name = first_employer["company"]
...
for sector in f500["sector"].unique():
    sector_check = f500["sector"] == sector
    companies = f500.loc[sector_check]
    first_company = companies.sort_values("roa", ascending=False).iloc[0]
    name = first_company["company"]
    top_roa_by_sector[sector] = name